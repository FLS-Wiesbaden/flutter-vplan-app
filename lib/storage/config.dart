...
class Config extends ChangeNotifier {
  ...
  static List<School> schools = [
    School("fls", "FLS-Wiesbaden", "assets/images/AppIcon.png", "https://www.fls-wiesbaden.de", "https://www.fls-wiesbaden.de/geco/auth"),
  ...
  if (!await _storage.containsKey(key: configKeyMode)) {
    ...
    return await _storage.read(key: configKeyMode) == configPlanTeacher ? PlanType.teacher : PlanType.pupil;
  }
  ...
  void setMode(PlanType pt) async {
    ...
    await _storage.write(
      key: configKeyMode, value: pt == PlanType.teacher ? configPlanTeacher : configPlanPupil
  ...
  void setAddRegularPlan(bool pt) async {
    _addRegularPlan = pt;
    await _storage.write(key: configKeyAddRegularPlan, value: pt ? 'true' : 'false').whenComplete(() => notifyListeners());
  }
  ...
  Future<int> getNumberDays() async {
    if (await _storage.containsKey(key: configKeyNumberDays)) {
      int? days = int.tryParse((await _storage.read(key: configKeyNumberDays))!);
      return days ?? 5;
    } else {
      return 5;
    }
  ...
  void setNumberDays({int days = 5}) async {
    _numberDays = days;
    await _storage.write(key: configKeyNumberDays, value: days.toString()).whenComplete(() => notifyListeners());
  }
  ...
  Future<void> setAuthUser(String? user) {
    final log = Logger(vplanLoggerId);
    return _storage.write(key: configKeyAuthUser, value: user).then((value) => log.fine("User $user saved."));
  }
  ...
  Future<bool> isFirstCall() {
    return _storage.read(key: configKeyFirstCall).then<bool>((value) => value == null || value != '1');
  }

  /// Set whether first call was successful.
  Future<void> setFirstCallDone(bool firstCallDone) {
    return _storage.write(key: configKeyFirstCall, value: firstCallDone ? '1' : '0');
  }
